...
namespace MicroSliver
{
    ...
    public class IoC : IIoC
    {
        ...
        public IMap GetMap<TContract>()
        {
            ...
            throw new Exception("MicroSliver is unable to find a mapping for interface of type " + contract.GetTypeInfo().Name + ".");
        }
        ...
        private IMap AddMap<TContract, TConcrete>()
        {
            ...
            lock (_lock)
            {
                ...
                {
                    throw new Exception("A mapping for interface of type " + contract.GetTypeInfo().Name + " already exists.  MicroSliver does not allow for multiple bindings of the same type.");
                }
                return _mappings[contract];
            }
        }
        ...
        private object Get(Type T)
        {
            if (T.GetTypeInfo().IsInterface)
            {
                ...
                {
                    throw new Exception("MicroSliver does not have a mapping for interface of type " + T.GetTypeInfo().Name + ".", ex);
                }
            }
            else if (T.GetTypeInfo().IsPrimitive || T.GetTypeInfo().IsValueType)
            {
        ...
        private void CacheCtorInfo(Type T)
        {
            lock (_lock)
            {
                ...
                {
                    var concreteCtor = T.GetTypeInfo().DeclaredConstructors.ToList()[0];
